...
class ApplitoolsTestResultHandlerCypress {
  constructor(
    testResult,
    viewKey,
    proxyServerUrl,
  ...
  async downloadImages(dir, type) {
    if (dir == undefined || !fs.existsSync(dir)) {
      console.log(
        `Directory was undefined or non-existent. Saving images to: ${process.cwd()}`
      );
      dir = process.cwd();
    } else {
  ...
  setProxy(
    ...
    const proxy = {};
    let url = new URL(
      uri.includes("://") ? uri : `http://${uri}:${proxyServerPort}`
    );
  ...
  async sendRequest(options, retry = 1, delayBeforeRetry = false) {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `ServerConnector retrying request with delay ${delayBeforeRetry}...`
        );
  ...
  async longRequestCheckStatus(response) {
    switch (response.status) {
      ...
      case HTTPSTATUSCODES.ACCEPTED: {
        ...
        const requestResponse = await this.longRequestLoop(
          options,
          LONGREQUESTDELAYMS
        );
      ...
      default: {
        throw new Error(
          `Unknown error during long request: ${JSON.stringify(response)}`
        );
      }
    }
  }
  ...
  createHttpOptionsWithHeaders(requestOptions) {
    ...
    options.headers["Eyes-Date"] = dateFormat(
      new Date(),
      DATEFORMATRFC1123,
      true
    );
...
exports.ApplitoolsTestResultHandlerCypress = ApplitoolsTestResultHandlerCypress;
